...
pub struct Settings {
    ...
    /// Encrypted Cursor User API Key (`crsr_...`). Falls back to the Cursor
    /// Agent CLI's `auth.json` when absent.
    pub cursor_key: Option<EncryptedSecret>,
...
impl From<&Settings> for SettingsView {
    fn from(s: &Settings) -> Self {
        Self {
            ...
            cursor_key_set: s.cursor_key.is_some(),
